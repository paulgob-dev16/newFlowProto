(function () {
  ...
  function freshState() {
    ...
    return {
      ...
      studio: { profileId: "", profileName: "", profileState: "not_configured", feedState: "not_generated", associationState: "not_associated", publishingState: "not_published" },
      cm360: {
        deliveryAssignments: [],
        createdPlacements: [],
  ...
  function restore() {
    ...
    try {
      ...
      const state = { ...fallback, ...saved, activationPackage: fallback.activationPackage, media: { ...fallback.media, ...(saved.media || {}) }, studio: { ...fallback.studio, ...(saved.studio || {}) }, cm360: { ...fallback.cm360, ...(saved.cm360 || {}) }, tags: { ...fallback.tags, ...(saved.tags || {}) }, active: false, modal: null, assignmentPopover: null };
  ...
  function targetingIssues(state) {
    ...
    if (state.activationPackage.defaultRequired && !hasDefault) issues.push({ id: "default", label: "Missing default creative", details: "A Default Creative with no targeting rules is required.", section: "assignments" });
  ...
  function validationChecks(state, referenceData) {
    ...
    const mappingChecks = dynamicMappingIssues(state).map((issue) => ({ ...issue, ready: false }));
    return [
      ...creativeChecks,
      ...targetingChecks,
      ...mappingChecks,
      { id: "profile", label: "Studio Profile created", ready: state.studio.profileState === "created", details: state.studio.profileState === "created" ? state.studio.profileName : "Create or select a Studio Profile.", section: "studio" },
      { id: "generated", label: "Dynamic feed generated", ready: ["generated", "uploaded"].includes(state.studio.feedState), details: ["generated", "uploaded"].includes(state.studio.feedState) ? `${activeStats.activeVariantCount} active variants and ${activeStats.activeRenditionCount} formats combined with mappings and targeting.` : "Generate the Studio Dynamic Feed.", section: "studio" },
      { id: "uploaded", label: "Dynamic feed uploaded", ready: state.studio.feedState === "uploaded", details: state.studio.feedState === "uploaded" ? "Dynamic Feed uploaded to the simulated Studio destination." : "Upload the Dynamic Feed.", section: "studio" },
      { id: "associated", label: "Creative associated", ready: state.studio.associationState === "associated", details: state.studio.associationState === "associated" ? "Approved creative package is associated with the Studio Profile." : "Associate the approved creatives.", section: "studio" },
      { id: "studio-published", label: "Studio publishing completed", ready: state.studio.publishingState === "published", details: state.studio.publishingState === "published" ? "Creative and feed published to Studio." : "Publish the completed Studio configuration.", section: "studio" },
      ...mediaChecks,
      ...deliveryChecks,
    ];
  }
  ...
  function invalidateStudioData(state) {
    state.studio.feedState = "not_generated";
    state.studio.associationState = "not_associated";
  ...
  function renderNavItem(state, section, index, referenceData) {
    ...
    if (section.id === "studio") { const done = state.studio.publishingState === "published"; const issues = dynamicMappingIssues(state).length; label = done ? "Published" : issues ? `${issues} mapping issues` : state.studio.profileState === "created" ? "In progress" : "Not configured"; tone = done ? "green" : issues ? "yellow" : state.studio.profileState === "created" ? "blue" : "gray"; }
  ...
  function renderStudio(state, referenceData) {
    const profileReady = state.studio.profileState === "created";
    const generated = ["generated", "uploaded"].includes(state.studio.feedState);
    const uploaded = state.studio.feedState === "uploaded";
    const inputsBlocked = [...targetingIssues(state), ...dynamicMappingIssues(state)].length > 0;
    const associated = state.studio.associationState === "associated";
    const studioPublished = state.studio.publishingState === "published";
    const studioAdvertiser = referenceById(referenceData, "studioAdvertisers", state.media.studioAdvertiserId);
    const mediaReady = Boolean(studioAdvertiser);
    const stats = activationStats(state);
    return `<div class="dco-section-head"><div><h2>Studio</h2><p>Prepare and publish the Dynamic Creative package to the simulated Google Studio destination.</p></div><div class="dco-prototype-label">Prototype only · No Studio API calls</div></div>${stats.inactiveRenditionCount ? `<section class="dco-exclusion-notice"><div>${chip("Excluded", "gray")}</div><div><strong>${stats.inactiveRenditionCount} inactive format${stats.inactiveRenditionCount === 1 ? "" : "s"} excluded from Studio</strong><p>${stats.activeVariantCount} active variant${stats.activeVariantCount === 1 ? "" : "s"} and ${stats.activeRenditionCount} active format${stats.activeRenditionCount === 1 ? "" : "s"} will be used.</p></div></section>` : ""}${renderDynamicMapping(state)}<section class="dco-panel ${mediaReady ? "" : "dco-panel-disabled"}"><div class="dco-panel-head"><div><span class="dco-kicker">2</span><h3>Studio Profile</h3></div>${chip(profileReady ? "Created" : mediaReady ? "Not created" : "Needs setup", profileReady ? "green" : mediaReady ? "gray" : "yellow")}</div>${mediaReady ? `<dl class="dco-inline-metadata"><div><dt>Profile</dt><dd>${escapeHtml(state.studio.profileName || "Not created")}</dd></div><div><dt>Advertiser</dt><dd>${escapeHtml(studioAdvertiser.name)}</dd></div></dl><div class="dco-action-row"><button class="primary-button" type="button" data-dco-action="create-profile">Create Studio Profile</button><button class="secondary-button" type="button" data-dco-action="select-profile">Select Existing Profile</button></div>` : '<div class="dco-dependency"><strong>Studio media connection required</strong><span>Select a Google Studio Advertiser in Media first.</span><button class="secondary-button" type="button" data-dco-action="section" data-section="media">Open Media</button></div>'}</section><section class="dco-panel ${profileReady ? "" : "dco-panel-disabled"}"><div class="dco-panel-head"><div><span class="dco-kicker">3</span><h3>Dynamic Feed</h3></div>${chip(state.studio.feedState === "not_generated" ? "Not generated" : uploaded ? "Uploaded" : "Generated", state.studio.feedState === "not_generated" ? "gray" : "green")}</div>${!profileReady ? '<div class="dco-dependency"><strong>Studio Profile required</strong><span>Create or select a profile first.</span></div>' : `<dl class="dco-inline-metadata"><div><dt>Active variants</dt><dd>${stats.activeVariantCount}</dd></div><div><dt>Active formats</dt><dd>${stats.activeRenditionCount}</dd></div><div><dt>Segments</dt><dd>${state.segments.filter((segment) => segment.enabled).length} enabled</dd></div><div><dt>Excluded</dt><dd>${state.activationPackage.excludedPreviewCount || 0} unapproved · ${stats.inactiveRenditionCount} inactive</dd></div></dl>${inputsBlocked ? `<div class="dco-dependency"><strong>Activation inputs require review</strong><span>Resolve Segment, assignment, and Dynamic Mapping issues before feed generation.</span></div>` : ""}<div class="dco-action-row"><button class="primary-button" type="button" data-dco-action="generate-feed" ${inputsBlocked ? "disabled" : ""}>Generate Feed</button><button class="secondary-button" type="button" data-dco-action="preview-feed" ${generated ? "" : "disabled"}>Preview Feed</button><button class="secondary-button" type="button" data-dco-action="upload-feed" ${generated && !uploaded ? "" : "disabled"}>Upload Feed</button></div>`}</section><section class="dco-panel ${uploaded ? "" : "dco-panel-disabled"}"><div class="dco-panel-head"><div><span class="dco-kicker">4</span><h3>Creative Association</h3></div>${chip(associated ? "Associated" : "Not configured", associated ? "green" : "gray")}</div>${uploaded ? `<dl class="dco-inline-metadata"><div><dt>Creative</dt><dd>${escapeHtml(state.activationPackage.name)}</dd></div><div><dt>Studio Profile</dt><dd>${escapeHtml(state.studio.profileName)}</dd></div><div><dt>Dynamic Feed</dt><dd>Uploaded</dd></div></dl><div class="dco-action-row"><button class="primary-button" type="button" data-dco-action="associate">Associate Creative</button></div>` : '<div class="dco-dependency"><strong>Dynamic Feed upload required</strong><span>Upload the feed before association.</span></div>'}</section><section class="dco-panel ${associated ? "" : "dco-panel-disabled"}"><div class="dco-panel-head"><div><span class="dco-kicker">5</span><h3>Studio Publishing</h3></div>${chip(studioPublished ? "Published" : "Not published", studioPublished ? "green" : "gray")}</div>${associated ? `<p class="dco-section-copy">Publish the associated approved creative and Dynamic Feed to Studio before the CM360 trafficking plan can be published.</p><div class="dco-action-row"><button class="primary-button" type="button" data-dco-action="publish-studio" ${studioPublished ? "disabled" : ""}>${studioPublished ? "Published to Studio" : "Publish to Studio"}</button></div>` : '<div class="dco-dependency"><strong>Creative Association required</strong><span>Associate the creative before Studio publishing.</span></div>'}</section>`;
  }
  ...
  function open(state, context) { state.active = true; state.section = state.section || "overview"; state.modal = null; syncMediaContext(state, context?.workspace, context?.referenceData); }
  ...
  function handleChange(event, state, context) {
    ...
    if (event.target.dataset.dcoMediaField) {
      ...
      if (field === "cmCampaignId" && previous !== event.target.value) { state.cm360.deliveryAssignments = []; state.cm360.createdPlacements = []; invalidateTrafficking(state); }
  ...
  window.cm360DcoActivationWorkflow = { createState: restore, open, render, handleClick, handleInput, handleChange };
})();
