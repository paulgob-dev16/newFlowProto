(function () {
  ...
  function targetingIssues(state) {
    ...
    if (state.activationPackage.defaultRequired && state.studio?.fallbackEnabled !== false && !activeDefault) issues.push({ id: "default", label: "Missing active default creative", details: "Turn on at least one format for the Default Creative or disable the Studio fallback rule.", section: "assignments" });
  ...
  function renderSegmentAssignmentPicker(state, variant, assignment, selectedSegment) {
    const open = state.assignmentPopover?.kind === "segment" && state.assignmentPopover.variantId === variant.id;
    const options = [{ id: "", name: "Default / No targeting", available: true }, ...state.segments.map((segment) => ({ id: segment.id, name: segment.name, available: segment.enabled && !model.segmentIssues(segment, state.segments).length }))];
    const selectedName = selectedSegment?.name || "Default / No targeting";
    return `<div class="dco-segment-select-cell"><button class="dco-segment-select-trigger" type="button" data-dco-action="toggle-assignment-segment-menu" data-variant-id="${variant.id}" aria-haspopup="listbox" aria-expanded="${open}"><span>${escapeHtml(selectedName)}</span><span aria-hidden="true">⌄</span></button>${selectedSegment ? `<button class="text-button" type="button" data-dco-action="edit-segment" data-segment-id="${selectedSegment.id}" data-variant-id="${variant.id}">View</button>` : ""}${open ? `<div class="dco-segment-select-menu" role="listbox" aria-label="Choose Segment">${options.map((option) => `<button type="button" role="option" aria-selected="${assignment.segmentId === option.id}" data-dco-action="choose-assignment-segment" data-variant-id="${variant.id}" data-segment-id="${option.id}" ${option.available ? "" : "disabled"}><span>${escapeHtml(option.name)}</span>${assignment.segmentId === option.id ? '<span class="dco-option-check">✓</span>' : option.available ? "" : '<span class="dco-option-state">Unavailable</span>'}</button>`).join("")}</div>` : ""}</div>`;
  }
  ...
  function renderValuePicker(state, modal) {
    ...
    let content = "";
    if (source === "connected") {
      const allObjects = type === "DV360 Line Item" ? fixtures.dv360LineItems : fixtures.cm360Placements;
      const objects = pickerObjects(type, modal), campaigns = [...new Set(allObjects.map((item) => item.campaign))], formats = [...new Set(fixtures.cm360Placements.map((item) => item.format))];
      const selectedCount = allObjects.filter((item) => current.has(item.id)).length, allSelected = Boolean(allObjects.length) && selectedCount === allObjects.length;
      content = `<div class="dco-picker-account"><span><strong>${type.startsWith("DV360") ? "DV360" : "CM360"}</strong><small>${escapeHtml(fixtures.accounts?.[type.startsWith("DV360") ? "DV360" : "CM360"]?.name || "Connected account")}</small></span>${chip("Connected", "green")}</div><div class="dco-picker-filters"><input data-segment-picker-field="search" value="${escapeHtml(modal.search || "")}" placeholder="Search name or platform ID"><select data-segment-picker-field="campaign"><option value="">All campaigns</option>${campaigns.map((campaign) => `<option ${modal.campaign === campaign ? "selected" : ""}>${escapeHtml(campaign)}</option>`).join("")}</select>${type === "CM360 Placement" ? `<select data-segment-picker-field="format"><option value="">All formats</option>${formats.map((format) => `<option ${modal.format === format ? "selected" : ""}>${escapeHtml(format)}</option>`).join("")}</select>` : ""}</div><div class="dco-table-scroll"><table class="dco-data-table dco-object-browser"><thead><tr><th><label class="dco-picker-select-all"><input type="checkbox" data-dco-action="segment-picker-select-all" ${allSelected ? "checked" : ""} ${allObjects.length ? "" : "disabled"}><span>Select all</span><small>${selectedCount} selected</small></label></th><th>${type === "DV360 Line Item" ? "Line item" : "Placement"}</th><th>${type === "DV360 Line Item" ? "Line Item ID" : "Placement ID"}</th><th>Campaign</th>${type === "CM360 Placement" ? "<th>Site</th><th>Format</th>" : "<th>Compatibility</th>"}</tr></thead><tbody>${objects.map((item) => `<tr><td><input type="checkbox" data-segment-picker-value="${item.id}" ${current.has(item.id) ? "checked" : ""}></td><td><strong>${escapeHtml(item.name)}</strong><small>${escapeHtml(item.source)}</small></td><td>${escapeHtml(item.platformId)}</td><td>${escapeHtml(item.campaign)}</td>${type === "CM360 Placement" ? `<td>${escapeHtml(item.site)}</td><td>${chip(item.format, "blue")}</td>` : `<td>${escapeHtml(item.compatibility || "All supported formats")}</td>`}</tr>`).join("") || '<tr><td colspan="6"><div class="table-empty"><strong>No matching connected objects</strong><span>Adjust the filters or use Manual.</span></div></td></tr>'}</tbody></table></div>`;
    } else if (source === "manual") {
      content = `<div class="dco-manual-value-form">${type === "Manual Entry" ? `<label class="field"><span>ID Type</span><select data-segment-picker-field="manualIdType">${model.MANUAL_ID_TYPES.map((idType) => `<option value="${escapeHtml(idType)}" ${modal.manualIdType === idType ? "selected" : ""}>${escapeHtml(idType)}</option>`).join("")}</select><small>The selected ID Type becomes this mapping's dynamic-feed column name.</small></label>` : ""}<label class="field"><span>${type === "Geography" ? "Locations" : type === "Manual Entry" ? "Values" : "Platform IDs"}</span><textarea rows="6" data-segment-picker-field="manualText" placeholder="One value per line or comma separated">${escapeHtml(modal.manualText || "")}</textarea><small>${type === "CM360 Placement" ? "Unknown IDs are kept in the Segment and feed, but block delivery while assigned." : "Values are de-duplicated when added."}</small></label></div>`;
    } else {
  ...
  function handleClick(event, state, context) {
    ...
    if (action === "edit-segment") { const segment = segmentById(state, target.dataset.segmentId); state.assignmentPopover = null; if (segment) { openEditor(state, segment, "edit"); const contextVariant = variantById(state, target.dataset.variantId); const contextFormats = contextVariant ? [...new Set(activeRenditionsFor(state, contextVariant).map((rendition) => rendition.format))] : []; if (contextFormats.length === 1) state.modal.format = contextFormats[0]; } context.render(); return true; }
    ...
    if (action === "segment-picker-select-all") {
      const allObjects = state.modal.mappingType === "DV360 Line Item" ? fixtures.dv360LineItems : fixtures.cm360Placements;
      const ids = new Set(state.modal.selectedValueIds || []);
      allObjects.forEach((item) => target.checked ? ids.add(item.id) : ids.delete(item.id));
      state.modal.selectedValueIds = [...ids];
      context.render(); return true;
    }
    ...
    if (action === "toggle-assignment") { const ids = new Set(state.expandedVariantIds); ids.has(target.dataset.variantId) ? ids.delete(target.dataset.variantId) : ids.add(target.dataset.variantId); state.expandedVariantIds = [...ids]; state.assignmentPopover = null; context.render(); return true; }
    if (action === "toggle-assignment-segment-menu") { state.assignmentPopover = state.assignmentPopover?.kind === "segment" && state.assignmentPopover.variantId === target.dataset.variantId ? null : { kind: "segment", variantId: target.dataset.variantId }; context.render(); return true; }
    if (action === "choose-assignment-segment") {
      const assignment = assignmentFor(state, target.dataset.variantId);
      assignment.segmentId = target.dataset.segmentId || "";
      state.assignmentPopover = null;
      if (assignment.segmentId) assignMappedPlacements(state, assignment, false);
      context.changed(assignment.segmentId ? "Segment assigned. Targeting and delivery validation updated." : "Segment removed from creative assignment.");
      return true;
    }
  ...
  function handleChange(event, state, context) {
    ...
    if (event.target.hasAttribute("data-segment-picker-value") && state.modal?.type === "segment-editor") { const ids = new Set(state.modal.selectedValueIds || []); event.target.checked ? ids.add(event.target.dataset.segmentPickerValue) : ids.delete(event.target.dataset.segmentPickerValue); state.modal.selectedValueIds = [...ids]; context.render(); return true; }
